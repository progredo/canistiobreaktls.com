...
If your app inside the mesh makes an outbound request to `https://thirdparty.com/api/v1`, Istio cannot look inside the encrypted traffic. If a client outside the mesh makes a request to `https://yourcompany.com/` and you have configured Istio to route TLS straight through to be terminated at your application pods, Istio cannot look inside that traffic either.
